...
const coffeeData = {
  types: [
    ...
    // PREMIUM TIER (50-75) - single origin - BLACK ONLY with roast and brew options
    {
      ...
      baseSyrups: 0,
      blackOnly: true,
      roasts: ['light', 'medium-light'],
      brewMethods: ['pour-over', 'aeropress', 'cold brew', 'filter'],
    },
    {
    ...
    },
    {
      ...
      baseSyrups: 0,
      blackOnly: true,
      roasts: ['light', 'super light'],
      brewMethods: ['pour-over', 'aeropress', 'french press'],
    },
    {
      ...
      blackOnly: true,
      roasts: ['light', 'medium'],
      brewMethods: ['french press', 'pour-over', 'auto drip', 'aeropress'],
    },
    {
      name: 'maui mokka',
      typeMin: 50,
      ...
      blackOnly: true,
      roasts: ['medium-light'],
      brewMethods: ['pour-over', 'french press', 'aeropress'],
    },
    {
  ...
  // Sugar-free syrups (for when sugar-free toggle is on)
  sugarFreeSyrups: [
    { name: 'sugar-free vanilla' },
    { name: 'sugar-free caramel' },
    { name: 'sugar-free hazelnut' },
    { name: 'sugar-free cinnamon dolce' },
    { name: 'sugar-free white mocha' },
    { name: 'sugar-free mocha' },
  ],

  // Sweeteners (for sugar level "none")
  sweeteners: [
    { name: 'splenda' },
    { name: 'stevia' },
    { name: 'monk fruit' },
  ...
  hiddenDrinks: [
    {
      name: "Scout's Special",
      description: "hidden achievement: you survived scout's coffee order",
      recipe:
    ...
    },
    {
      ...
      description:
        'special drink for birthday, tiramisu drink with extra espresso',
      recipe:
        'Blended iced latte with hazelnut, toffee nut, caramel, vanilla, and mocha, 2 shots espresso affogato, topped with whipped cream, caramel and mocha drizzle',
      trigger: {
        type: 'classic',
        base: 'latte',
        sugar: 4,
    ...
    },
    {
      ...
      recipe:
        'Double espresso with coconut milk, 2 pumps hazelnut, and 1 pump vanilla',
      trigger: {
        type: 'classic',
        base: 'espresso',
        sugar: 3,
    ...
    {
      name: 'Matcha Kit Kat',
      description: 'Tastes just like the Japanese candy!',
      recipe: 'Blended iced matcha latte with white mocha and soy milk',
      trigger: {
        type: 'unique',
        base: 'matcha latte',
        sugar: 1,
...
const sugarFreeCheck = document.getElementById('sugarFree'); // New sugar-free toggle
...
function generateDrink() {
  ...
  const sugarFree = sugarFreeCheck.checked; // New sugar-free toggle
  ...
  if (base.blackOnly) {
    ...
    const roastLevel = base.roasts
      ? base.roasts[Math.floor(Math.random() * base.roasts.length)]
      : 'light';

    // Select random brew method from available options
    const brewMethod = base.brewMethods
      ? base.brewMethods[Math.floor(Math.random() * base.brewMethods.length)]
      : 'pour-over';
  ...
  if ((isCoffee || isTea) && !isBlended && Math.random() > 0.3) {
    const hasMilkInName = base.name.includes('milk') ||
    ...
    if (!hasMilkInName) {
      ...
      const baseHasDairyMilk = base.name.includes('milk') ||
        base.name.includes('latte') ||
        base.name.includes('flat white') ||
        base.name.includes('cappuccino') ||
        base.name.includes('cortado') ||
      ...
      if (baseHasDairyMilk) {
        const dairyFreeMilks = coffeeData.milks.filter((m) =>
          m.dairyFree && m.name !== 'no milk'
        );
        if (dairyFreeMilks.length > 0) {
          const milk =
            dairyFreeMilks[Math.floor(Math.random() * dairyFreeMilks.length)];
  ...
  // Add sweeteners for sugar level "none" (always, not random)
  if (
    sugarLevel === 0 && (isCoffee || isTea) && coffeeData.sweeteners.length > 0
  ) {
    const sweetener = coffeeData
      .sweeteners[Math.floor(Math.random() * coffeeData.sweeteners.length)];
    orderParts.push(`sweetened with ${sweetener.name}`);
  } // Add syrups for sugar levels > 0
  else if (sugarLevel > 0 && (isCoffee || isTea)) {
    ...
    if (totalDesiredPumps > baseSyrups) {
      ...
      // Choose between regular syrups or sugar-free syrups based on toggle
      let syrupSource;
      if (sugarFree && coffeeData.sugarFreeSyrups.length > 0) {
        syrupSource = coffeeData.sugarFreeSyrups;
      } else {
        syrupSource = coffeeData.syrups;
      }

      const syrup = syrupSource[Math.floor(Math.random() * syrupSource.length)];
      ...
      if (sugarLevel === 4 && Math.random() > 0.5) {
        const secondSyrup =
          syrupSource[Math.floor(Math.random() * syrupSource.length)];
  ...
  if (caffeineLevel > 0) {
    ...
    if (base.name === 'espresso') {
      ...
      (base.category === 'tea' || base.name.includes('tea')) && totalShots > 0
    ) {
      ...
      if (extraShots > 0) {
        if (extraShots === 1) {
          if (Math.random() > 0.7) {
            const shotType = coffeeData
              .espresso[
                Math.floor(Math.random() * coffeeData.espresso.length)
              ];
